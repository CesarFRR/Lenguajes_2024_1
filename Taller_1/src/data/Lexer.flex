...
%{
    ...
    List<String> operadoresRelacionales_OP_COND = Arrays.asList("<", "<=", ">", ">=", "==", "!=");
    ...
    List<String> delimitadoresComillas_DEL_COM = Arrays.asList("\"", "'");
...
<YYINITIAL> {
    ...
    "=" { return token(sym.OP_ASIG, yytext(), yyline, yycolumn); }

    /* Estructuras condicionales */
    "if" { return token(sym.COND_IF, yytext(), yyline, yycolumn); }
    "elif" { return token(sym.COND_ELIF, yytext(), yyline, yycolumn); }
    "else" { return token(sym.COND_ELSE, yytext(), yyline, yycolumn); }
